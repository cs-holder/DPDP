...
CIMAAct = {"Confirmation": "Please confirm  or understanding is correct.",
...
def ESConvMessages(case, role, conversation, action=None):
    if role == 'system':
        messages = [{"role":"system", "content":"Now enter the role-playing mode. In the following conversation, you will play as a therapist in a counselling conversation with a patient."}, 
                    {"role":"PATIENT", "content": "You are the therapist who is trying to help the patient reduce their emotional distress and help them understand and work through the challenges. Please reply with only one short and succinct sentence. %s Are you ready to play the game?" % ESConvAct[action]}, 
                    {"role":"Therapist", "content":"Yes, I'm ready to play the game!"}]
        messages.extend(conversation)
    elif role == 'user':
        messages = [{"role":"system", "content":"Now enter the role-playing mode. In the following conversation, you will play as a patient in a counselling conversation with a therapist."}, 
        ...
        messages = [{"role":"system", "content":"Given a conversation between a Therapist and a Patient, please assess whether the Patient' emotional issue has been solved after the conversation."}, 
...
def CIMAMessages(case, role, conversation, action=None):
    if role == 'system':
        messages = [{"role":"system", "content":"Now enter the role-playing mode. In the following conversation, you will play as a teacher in a tutoring conversation with a student."}, 
                    {"role":"Student", "content": "You are the teacher who is trying to teach the student to translate \"%s\" into Italian. Please reply with only one short and succinct sentence. Please do not tell the student the answer or ask the student about other exercises. %s Now ask me an exercise." % (case['sentence'], CIMAAct[action])}]
        messages.extend(conversation)
    elif role == 'user':
        messages = [{"role":"system", "content":"Now enter the role-playing mode. In the following conversation, you will play as a student who does not know Italian in a tutoring conversation with a teacher."}, 
                    {"role":"teacher", "content":"You are the student who is trying to translate a English sentence into Italian. You don't know the translation of \"%s\" in Italian. Please reply with only one short and succinct sentence. Are you ready to play the game?" % case['sentence']}, 
                    {"role":"student", "content":"Yes, I'm ready to play the game!"}]
        messages.extend(conversation)
    elif role == 'critic':
        ...
        messages = [{"role":"system", "content":"Given a conversation between a Teacher and a Student, please assess whether the Student correctly translate the English sentence into Italian in the conversation."}, 
...
def chatglm_prompt(messages, role):
    #print(messages)
    new_messages = [messages[0]]
    for message in messages[1:]:
        if message['role'] == role:
