...
class CIMAChatSystemPlanner(CIMASystemPlanner):
    ...
    def _get_generated_da(self, data) -> list:
        # convert generated responses to DA
        action_list = [CIMAGame.S_Confirmation, CIMAGame.S_Correction, CIMAGame.S_Hint, CIMAGame.S_OpenEndedQuestion, CIMAGame.S_Others]
        pred_da = []
        for resp in data:
            resp = resp["generated_text"].strip()
            start_idx = resp.find("[")
    ...
    def predict(self, state: DialogSession, policy, ent_bound, agent_state) -> "Tuple[np.ndarray, float]":
        ...
        # the value would be our objective function
        with torch.no_grad():
            agent_dist, _ = policy.action_policy(agent_state)
...
class TherapistModel(DialogModel):
    def __init__(
        ...
        self.da_prompts_mapping = {
            CIMAGame.S_Confirmation: "The Teacher confirms the Student's answer or understanding is correct.",
            CIMAGame.S_Correction: "The Teacher corrects a mistake or addresses a misconception the Student has.",
            CIMAGame.S_Hint: "The Teacher provides knowledge to the Student via a hint.",
            CIMAGame.S_OpenEndedQuestion: "The Teacher asks a question to the Student to determine the Student's understanding or continue the conversation.",
            CIMAGame.S_Others: "The Teacher chats with the Student.",
        }
    ...
    def get_utterance_batched(
        ...
        if self.zero_shot:
            messages = [
                ...
                {"role": "user", "content": "You are the teacher who is trying to teach the student to translate '%s' into Italian. Please reply with only one short and succinct sentence. Please do not tell the student the answer or ask the student about other exercises. %s Now ask me an exercise." % (da_prompt)}
            ]
        else:
            messages = [
                {"role": "system", "content": self.task_prompt},
...
class PatientChatModel(PatientModel):
    ...
    def get_utterance(self, state: DialogSession, action=None, mode='train') -> str:
        ...
        if self.zero_shot:
            messages = [
                {"role": "system", "content": self.task_prompt},
                {"role": "user", "content": "You are the student who is trying to translate an English sentence into Italian. You don't know the translation of '%s' in Italian. Please reply with only one short and succinct sentence. Are you ready to play the game?" % (state)},
