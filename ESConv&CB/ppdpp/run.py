...
success_base_map = {'esc': reward_dict['esc']['better'], 'cb': 0.4}
start_reward = {'esc': -0.5, 'cb': -1.0}
...
def train(args, config, dataset, filename, tokenizer):
    ...
    policy = PPDPP(args, config, tokenizer, success_base_map[args.data_name]) # policy network init
    ...
    for train_step in range(args.start_step + 1, args.max_steps+1):
        ...
        for i_episode in tqdm(range(args.sample_times),desc='sampling'):
            ...
            for t in count():   # user  dialog
                ...
                if full_mcts_history is None:
                    ...
                    try:
                        ...
                        print('state: ', state)
                        print('system role: ', system_role[args.data_name])
                        print('error')
                        raise ValueError
                ...
                if done:
                    ...
                    rewards = [start_reward[args.data_name]] + policy.rewards
            ...
            if (i_episode + 1) % args.target_update_count == 0:
                policy.update_target_qnet()
...
def evaluate(args, dataset, policy, filename, i_episode, train_env, mode='valid'):
    ...
    with torch.no_grad():
        for test_num in tqdm(range(start_index, test_size)):  #test_size
            ...
            full_mcts_history, mcts_turn, rewards = None, 0, [start_reward[args.data_name]]
            transition_dict = {'states': [], 'actions': [], 'next_states': [], 'rewards': [], 'dones': []}
            for t in count():  # user  dialog
                if full_mcts_history is None:
    ...
    with open(PATH, 'a') as f:
        ...
        f.write('Apply chatgpt times: {}\n'.format(policy.apply_chatgpt_times + test_env.apply_chatgpt_times))
    ...
    for i_episode in range(args.start_step + 1, args.max_steps+1):
        ...
        with torch.no_grad():
            for test_num in tqdm(range(start_index, start_index + test_size)):  #test_size
                ...
                if (test_num - start_index) % 50 == 0:
                    logger.info('Eval action freq: {}'.format(str(policy.action_freq)))
                logger.info('Current SR: {}'.format(SR / (test_num - start_index + 1)))
                logging.info('Current action freq: {}'.format(json.dumps(action_dict)))
...
def record_trajectories(args, state, mcts_state, rewards, transition_dict, mode):
    assert len(state) == len(rewards) * 2 - 1
    trajectory, idx = [], 0
    for role, act, uttr in mcts_state.history[: len(state) + 1]:
    ...
    with open(os.path.join(args.output_dir, '{}_trajectory.txt'.format(mode)), 'a+', encoding='utf-8') as fout:
        diag = {"emotion_type": mcts_state.emotion_type, "problem_type": mcts_state.problem_type, "situation": state[0]['content'], 
                "dialog": trajectory}
        json.dump(diag, fout)
        fout.write('\n')
...
def parse_args():
    ...
    parser.add_argument('--checkpoint_path', default=None, #../pretrain/outputs/best_pretrain.pt
                        type=str, help="checkpoint model path.")
    ...
    parser.add_argument("--neg_reward", type=float, default=-1.0)
...
def main(args):
    ...
    args.output_dir = os.path.join(args.output_dir, args.data_name, trained, 
                                   "{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}".format(args.model_name, args.system, args.user, args.critic, args.max_steps, 
                                                                                         args.sample_times, args.eval_sample_times, args.learning_rate, args.mcts_applied_ratio, 
                                                                                         args.target_update_count, args.dropout, args.critic_loss_w, 
                                                                                         args.eps, args.neg_reward, args.remark))
    ...
    if args.do_train or args.do_eval:
        filename = '{}-{}-{}-{}-{}-{}-{}-{}-{}-{}'.format(args.data_name, args.sft_dir, args.system, args.user, args.critic, args.mcts_applied_ratio, 
                                                          args.max_steps, args.target_update_count, args.dropout, args.eps)
    elif args.do_epoch_eval:
