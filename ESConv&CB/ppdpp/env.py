...
from mcts.core.helpers import EmotionSupportDialogSession, CIMADialogSession, CBDialogSession
...
class Env(object):
    ...
    def reset(self):
        ...
        if self.args.data_name == 'esc':
            ...
            self.conversation = [{"role":"Buyer", "content":"Hi, how much is the %s?" % self.case['item_name']}, 
                                 {"role":"Seller", "content":"Hi, this is a good %s and its price is %s." % (self.case['item_name'], self.case['seller_price'])}]
        self.logger.info(json.dumps(self.conversation))
        if self.args.data_name == 'esc':
            history = [(system_role[self.args.data_name], EmotionalSupportGame.S_Others, 'Hi!'), 
                    (user_role[self.args.data_name], EmotionalSupportGame.U_FeelTheSame, self.case['situation'])]
            return self.conversation, EmotionSupportDialogSession(system_role[self.args.data_name], user_role[self.args.data_name], 
                                                                self.case['emotion_type'], self.case['problem_type'], history=history)
        elif self.args.data_name == 'cb':
            history = [(system_role[self.args.data_name], CBGame.S_Inquire, "Hi, how much is the %s?" % self.case['item_name']), 
                    (user_role[self.args.data_name], CBGame.U_No_deal, "Hi, this is a good %s and its price is %s." % (self.case['item_name'], self.case['seller_price']))]
            return self.conversation, CBDialogSession(system_role[self.args.data_name], user_role[self.args.data_name], 
                                                      self.case['item_name'], self.case['buyer_item_description'], self.case['buyer_price'], 
                                                      self.case['seller_item_description'], self.case['seller_price'], history=history)
    ...
    def step(self, action, mcts_state, reward, use_mcts):
        ...
        if self.args.data_name == 'esc':
            ...
            thre = 0.0 if self.mode == 'test' else 0.4
            if reward >= thre:
                self.logger.info('--> Goal completed !')
                done = 1
            else:
    ...
    def compute_reward(self, model, messages, case):
        ...
        if self.args.data_name in ['esc','cima']:
            ...
            if -1 in deals:
                reward = self.args.neg_reward
                max_freq_action = 'no deal'
            else:
                if len(rewards) == 0:
                    reward = 0
                else:
                    reward = max(set(rewards), key = rewards.count)
                max_freq_action = 'deal'
            self.logger.info(str(reward))
