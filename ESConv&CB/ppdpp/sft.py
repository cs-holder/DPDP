...
success_base_map = {'esc': reward_dict['esc']['better'], 'cb': 0.0}
...
class DataFrame(Dataset):
    def __init__(self, data, args):
        ...
        self.target_qvs = data['target_qvs']
        self.max_len = args.max_seq_length

    def __getitem__(self, index):
        return self.state_ids[index][:self.max_len], self.action_ids[index], self.next_state_ids[index][:self.max_len], self.dones[index], self.rewards[index], self.target_qvs[index]
...
def collate_fn(data):
    state_ids, action_ids, next_state_ids, dones, rewards, target_qvs = zip(*data)
    ...
    target_qvs = torch.tensor(target_qvs).float()
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {'state_ids': batch['state_ids'].to(args.device), 
                    ...
                    #   'rewards': batch['rewards'].to(args.device),
                      'target_qvs': batch['target_qvs'].to(args.device)}
...
def epoch_evaluate(args, policy, i_episode, base_dir):
    ...
    with torch.no_grad():
        for test_num in tqdm(range(start_index, start_index + test_size)):  #test_size
            ...
            full_mcts_history, mcts_turn, rewards = None, 0, [mid_reward_map[args.data_name]]
...
def main(args):
    ...
    model = PPDPP(args, config, tokenizer, success_base_map[args.data_name])
